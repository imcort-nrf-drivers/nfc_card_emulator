...
static uint8_t Test_Memory_Type2[135 * 4] = {
    0xc0,0x58,0xc8,0xb5, // Internal 0-3
...
static void nfc_received_process(const uint8_t * p_data, size_t data_length)
{
		...
		switch (command)
		{
			case NFC_CMD_READ:
				...
				u8log_WriteString(&u8log, "Read Block\n");
				if (block_num < 135)
			...
			case NFC_CMD_WRITE:
				...
				u8log_WriteString(&u8log, "Write Block\n");
				if (data_length == 6) 
				{
						for (int i=0;i<4;i++)
						{
			...
			case NFC_CMD_PWD_AUTH:
				...
				u8log_WriteString(&u8log, "Pwd Auth\n");
				hal_nfc_send(NTAG215_PwdOK, 2);
				break;
			
			default:
				NRF_LOG_INFO("NFC CMD %x", p_data[0]);
				u8log_WriteString(&u8log, "Unknown CMD\n");
				break;	
